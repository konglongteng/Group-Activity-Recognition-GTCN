...
def train_net(cfg):
    ...
    #@kong add volleytatic
    basenet_list={'volleyball':Basenet_volleyball, 'collective':Basenet_collective,'volleytatic':Basenet_volleytatic}
    gcnnet_list={'volleyball':GCNnet_volleyball, 'collective':GCNnet_collective, 'volleytatic':GCNnet_volleytatic}
    ...
    #@kong add volleytatic
    train_list={'volleyball':train_volleyball, 'collective':train_collective, 'volleytatic':train_volleytatic}
    test_list={'volleyball':test_volleyball, 'collective':test_collective, 'volleytatic':test_volleytatic}
...
def train_volleytatic(data_loader, model, device, optimizer, epoch, cfg):
    actions_meter = AverageMeter()
    activities_meter = AverageMeter()
    loss_meter = AverageMeter()
    epoch_timer = Timer()
    ...
    for batch_data in data_loader:
        ...
        # Total loss
        #@kong change the loss, only the activities loss work
        #total_loss = activities_loss + cfg.actions_loss_weight * actions_loss
        ...
        if indexofiter % 100 == 0:
            print('Loss: %.5f, Using %.1f seconds' % (train_info_iter['loss'], train_info_iter['time']))
        indexofiter = indexofiter + 1
...
def test_volleytatic(data_loader, model, device, epoch, cfg):
    model.eval()
