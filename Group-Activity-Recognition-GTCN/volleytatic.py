...
class VolleytaticDataset(data.Dataset):
    ...
    """
    def __init__(self,anns,tracks,frames,images_path,det_path,image_size,feature_size,num_boxes=12,num_before=4,num_after=4,is_training=True,is_finetune=False):
        self.anns=anns
    ...
    def volley_frames_sample(self,frame):
        ...
        if self.is_finetune:
            if self.is_training:
                ...
                return [(sid, src_fid, fid)
                        for fid in range(src_fid, src_fid+num_offrames-1, int(num_offrames/15))]
        else:
            if self.is_training:
                ...
                #@kong
                return [(sid, src_fid, fid)
                        for fid in  [src_fid,src_fid+int(num_offrames/15), src_fid+int(num_offrames*2/15),src_fid+int(num_offrames*3/15),src_fid+int(num_offrames*4/15), src_fid+int(num_offrames*5/15),src_fid+int(num_offrames*6/15),src_fid+int(num_offrames*7/15),src_fid+int(num_offrames*8/15)]]
